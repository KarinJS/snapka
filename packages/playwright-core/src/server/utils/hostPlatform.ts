...
function calculatePlatform (): { hostPlatform: HostPlatform, isOfficiallySupportedPlatform: boolean } {
  ...
  if (platform === 'linux') {
    if (!['x64', 'arm64'].includes(os.arch())) { return { hostPlatform: '<unknown>', isOfficiallySupportedPlatform: false } }
    ...
    if (distroInfo?.id === 'ubuntu' || distroInfo?.id === 'pop' || distroInfo?.id === 'neon' || distroInfo?.id === 'tuxedo') {
      ...
      if (major < 20) { return { hostPlatform: ('ubuntu18.04' + archSuffix) as HostPlatform, isOfficiallySupportedPlatform: false } }
      if (major < 22) { return { hostPlatform: ('ubuntu20.04' + archSuffix) as HostPlatform, isOfficiallySupportedPlatform: isUbuntu && version === '20.04' } }
      if (major < 24) { return { hostPlatform: ('ubuntu22.04' + archSuffix) as HostPlatform, isOfficiallySupportedPlatform: isUbuntu && version === '22.04' } }
      if (major < 26) { return { hostPlatform: ('ubuntu24.04' + archSuffix) as HostPlatform, isOfficiallySupportedPlatform: isUbuntu && version === '24.04' } }
    ...
    if (distroInfo?.id === 'linuxmint') {
      ...
      if (mintMajor <= 20) { return { hostPlatform: ('ubuntu20.04' + archSuffix) as HostPlatform, isOfficiallySupportedPlatform: false } }
      if (mintMajor === 21) { return { hostPlatform: ('ubuntu22.04' + archSuffix) as HostPlatform, isOfficiallySupportedPlatform: false } }
    ...
    if (distroInfo?.id === 'debian' || distroInfo?.id === 'raspbian') {
      ...
      if (distroInfo?.version === '11') { return { hostPlatform: ('debian11' + archSuffix) as HostPlatform, isOfficiallySupportedPlatform } }
      if (distroInfo?.version === '12') { return { hostPlatform: ('debian12' + archSuffix) as HostPlatform, isOfficiallySupportedPlatform } }
      if (distroInfo?.version === '13') { return { hostPlatform: ('debian13' + archSuffix) as HostPlatform, isOfficiallySupportedPlatform } }
      ...
      if (distroInfo?.version === '') { return { hostPlatform: ('debian13' + archSuffix) as HostPlatform, isOfficiallySupportedPlatform } }
    }
    return { hostPlatform: ('ubuntu24.04' + archSuffix) as HostPlatform, isOfficiallySupportedPlatform: false }
  }
...
function toShortPlatform (hostPlatform: HostPlatform): ShortPlatform {
  ...
  if (hostPlatform.startsWith('mac')) { return hostPlatform.endsWith('arm64') ? 'mac-arm64' : 'mac-x64' }
