...
export async function installDependenciesLinux (targets: Set<DependencyGroup>, dryRun: boolean) {
  ...
  if (!isOfficiallySupportedPlatform) { console.warn(`BEWARE: your OS is not officially supported by Playwright; installing dependencies for ${platform} as a fallback.`) }
...
export async function transformCommandsForRoot (commands: string[]): Promise<{ command: string, args: string[], elevatedPermissions: boolean }> {
  ...
  if (isRoot) { return { command: 'sh', args: ['-c', `${commands.join('&& ')}`], elevatedPermissions: false } }
  const sudoExists = await spawnAsync('which', ['sudo'])
  if (sudoExists.code === 0) { return { command: 'sudo', args: ['--', 'sh', '-c', `${commands.join('&& ')}`], elevatedPermissions: true } }
