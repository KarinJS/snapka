...
import { dockerVersion, readDockerVersionSync, transformCommandsForRoot, installDependenciesLinux, installDependenciesWindows, validateDependenciesLinux, validateDependenciesWindows } from './dependencies'
...
export class Registry {
  ...
  constructor (browsersJSON: BrowsersJSON) {
    ...
    this._executables.push({
      ...
      _isHermeticInstallation: true,
      _install: async () => {
        if (process.platform !== 'win32') { throw new Error('WebKit via WSL is only supported on Windows') }
  ...
  private _createBidiFirefoxChannel (name: BidiChannel, lookAt: Record<'linux' | 'darwin' | 'win32', string>, install?: () => Promise<void>): ExecutableImpl {
    const executablePath = (sdkLanguage: string, shouldThrow: boolean) => {
      ...
      if (shouldThrow) { throw new Error(`Cannot find Firefox installation for channel '${name}' at the standard system paths. ${`Tried paths:\n  ${prefixes.map(p => path.join(p, suffix)).join('\n  ')}`}`) }
      return undefined
    }
    return {
      type: 'channel',
  ...
  private async _validateHostRequirements (sdkLanguage: string, browserDirectory: string, linuxLddDirectories: string[], dlOpenLibraries: string[], windowsExeAndDllDirectories: string[]) {
    ...
    if (os.platform() === 'win32' && os.arch() === 'x64') { return await validateDependenciesWindows(sdkLanguage, windowsExeAndDllDirectories.map(d => path.join(browserDirectory, d))) }
  }
  ...
  async install (executablesToInstall: Executable[], options?: { force?: boolean }) {
    ...
    try {
      ...
      if (!getAsBooleanFromENV('PLAYWRIGHT_SKIP_BROWSER_GC')) { await this._validateInstallationCache(linksDir) }
  ...
  private async _downloadExecutable (descriptor: BrowsersJSONDescriptor, executablePath?: string) {
    ...
    if (!downloadURLs.length) { throw new Error(`ERROR: Playwright does not support ${descriptor.name} on ${hostPlatform}`) }
    if (!isOfficiallySupportedPlatform) { logPolitely(`BEWARE: your OS is not officially supported by Playwright; downloading fallback build for ${hostPlatform}.`) }
    if (descriptor.hasRevisionOverride) {
      ...
      if (process.env.GITHUB_ACTIONS) { console.log(`::warning title=Playwright::${message}`) } else { logPolitely(message) }
    }
  ...
  private async _deleteStaleBrowsers (browserList: BrowserInfo[]) {
    ...
    for (const browser of browserList) {
      ...
      if (!shouldHaveMarkerFile || (await existsAsync(browserDirectoryToMarkerFilePath(browserPath)))) { usedBrowserPaths.add(browserPath) }
    }
  ...
  private async _deleteBrokenInstallations (brokenLinks: string[]) {
    for (const linkPath of brokenLinks) { await fs.promises.unlink(linkPath).catch(e => { }) }
  }

  private _defaultBrowsersToInstall (options: { shell?: 'no' | 'only' }): Executable[] {
    let executables = this.defaultExecutables()
    if (options.shell === 'no') { executables = executables.filter(e => e.name !== 'chromium-headless-shell') }
    if (options.shell === 'only') { executables = executables.filter(e => e.name !== 'chromium') }
    return executables
  }
  ...
  resolveBrowsers (aliases: string[], options: { shell?: 'no' | 'only' }): Executable[] {
    ...
    const handleArgument = (arg: string) => {
      ...
      if (executable?.browserName === 'chromium') { executables.push(this.findExecutable('ffmpeg')!) }
    }
    ...
    if (faultyArguments.length) { throw new Error(`Invalid installation targets: ${faultyArguments.map(name => `'${name}'`).join(', ')}. Expecting one of: ${this.suggestedBrowsersToInstall()}`) }
    return executables
  }
}
