...
export interface PuppeteerConnectOptions extends Omit<SnapkaConnectOptions, 'defaultViewport'>, ConnectOptions {
 ...
 * 负责浏览器路径解析、查找和下载
 */
export class SnapkaLaunch {
  /**
   * 获取浏览器可执行文件路径
   * @param options - 启动选项
   * @returns 浏览器路径或 null
   */
  async getPath (options?: PuppeteerLaunchOptions): Promise<string | null> {
   ...
   */
  private async download (config?: PuppeteerLaunchOptions['download']): Promise<string> {
