...
describe('SnapkaLaunch', () => {
  let snapkaLaunch: SnapkaLaunch
  ...
  describe('getPath', () => {
    it('should return custom path when provided and valid', async () => {
      ...
      const result = await snapkaLaunch.getPath({ executablePath: customPath })
    ...
    it('should return null when custom path is invalid', async () => {
      ...
      await expect(snapkaLaunch.getPath({ executablePath: '/invalid/path' }))
        .rejects.toThrow('executablePath 指定的路径不存在')
    })

    it('should throw error when custom path is not a string', async () => {
      await expect(snapkaLaunch.getPath({ executablePath: 123 as any }))
    ...
    it('should find browser when findBrowser is true', async () => {
      ...
      const result = await snapkaLaunch.getPath({ findBrowser: true })
    ...
    it('should return null when findBrowser is false and download is disabled', async () => {
      ...
      const result = await snapkaLaunch.getPath({ findBrowser: false, download: { enable: false } })
    ...
    it('should download browser when not found and download is enabled', async () => {
      ...
      const result = await snapkaLaunch.getPath({ download: { enable: true } })
  ...
  describe('validatePath', () => {
    it('should return null when path is null or undefined', () => {
      expect((snapkaLaunch as any).validatePath(null)).toBe(null)
      expect((snapkaLaunch as any).validatePath(undefined)).toBe(null)
    })

    it('should throw TypeError when path is not a string', () => {
      expect(() => (snapkaLaunch as any).validatePath(123)).toThrow('executablePath 仅支持 string 类型')
      expect(() => (snapkaLaunch as any).validatePath({})).toThrow('executablePath 仅支持 string 类型')
    })

    it('should throw Error when path does not exist', () => {
      mockFs.existsSync.mockReturnValue(false)
      expect(() => (snapkaLaunch as any).validatePath('/invalid/path'))
    ...
    it('should return path when path exists', () => {
      ...
      mockFs.existsSync.mockReturnValue(true)
      expect((snapkaLaunch as any).validatePath(validPath)).toBe(validPath)
    })
  })
  ...
  describe('find', () => {
    it('should find headed browsers first on Windows in debug mode', async () => {
      ...
      const result = await (snapkaLaunch as any).find(true)
    ...
    it('should find headless browsers first on non-Windows platforms', async () => {
      ...
      const result = await (snapkaLaunch as any).find(false)
  ...
  describe('runFinders', () => {
    it('should return first found browser path', async () => {
      ...
      const result = await (snapkaLaunch as any).runFinders(finders as any)
  ...
  describe('download', () => {
    it('should download browser when platform is supported', async () => {
      ...
      const result = await (snapkaLaunch as any).download()
    ...
    it('should throw error when platform is not supported', async () => {
      ...
      await expect((snapkaLaunch as any).download())
  ...
  describe('getBrowserType', () => {
    it('should return chromium by default on Windows platforms', () => {
      ...
      const result = (snapkaLaunch as any).getBrowserType()
    ...
    it('should return custom browser type when provided', () => {
      expect((snapkaLaunch as any).getBrowserType('chrome')).toBe('chrome')
      expect((snapkaLaunch as any).getBrowserType('chromium')).toBe('chromium')
      expect((snapkaLaunch as any).getBrowserType('chrome-headless-shell')).toBe('chrome-headless-shell')
    })

    it('should throw error for unsupported browser types', () => {
      expect(() => (snapkaLaunch as any).getBrowserType('firefox'))
  ...
  describe('getBaseUrl', () => {
    it('should return custom base url when provided', async () => {
      ...
      const launchInstance = new SnapkaLaunch()
