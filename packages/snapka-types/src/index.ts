...
export interface Snapka {
  /**
   * 启动浏览器实例
   * @param options 启动配置项
   */
  launch (options?: LaunchOptions): Promise<SnapkaCore>
  /**
   * 连接已启动的浏览器实例
   * @param options 连接配置项
   */
  connect (options: ConnectOptions): Promise<SnapkaCore>
}

/**
 * 浏览器实例
 */
export interface SnapkaCore {
  /**
   * 获取当前使用的浏览器引擎类型
   */
  engine: string
  ...
  /**
   * 获取当前浏览器二进制路径
   * @description 需要注意，在ws连接模式下无法获取此路径
   */
   ...
   * @param options 截图参数
   */
  screenshot<T extends 'base64' | 'binary'> (
    options: SnapkaScreenshotOptions<T>
  ): {
    /** 执行截图 */
    run: () => Promise<T extends 'base64' ? string : Buffer>
    /** 当前页面上下文 */
    context: any
  }
   ...
   * @param options 截图参数
   */
  screenshotViewport<T extends 'base64' | 'binary'> (
    options: SnapkaScreenshotOptions<T> & { viewportHeight?: number }
  ): {
    /** 执行截图 */
    run: () => Promise<T extends 'base64' ? string[] : Buffer[]>
    /** 当前页面上下文 */
