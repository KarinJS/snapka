...
基于 Puppeteer 的独立 Express 截图服务，采用单例浏览器架构，提供高性能的截图 API。
...
- `package.json` 的 `puppeteer-express` 字段
- `.puppeteer-expressrc`
- `.puppeteer-expressrc.json`
- `.puppeteer-expressrc.js`
- `puppeteer-express.config.js`
...
5. **WebP 优势**：相比 PNG，WebP 格式文件更小，适合大批量截图

## 🆚 与 Playwright 版本的区别

| 特性 | Puppeteer | Playwright |
|------|-----------|-----------|
| 引擎 | Puppeteer | Playwright |
| WebP支持 | ✅ 原生支持 | ❌ 转换为PNG |
| 性能 | 快 | 略快 |
| API | 完全一致 | 完全一致 |

两个版本的 API 完全相同，可以无缝切换。选择 Puppeteer 版本的优势是原生支持 WebP 格式。
